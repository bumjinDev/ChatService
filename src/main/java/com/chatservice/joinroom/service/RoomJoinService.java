...
public class RoomJoinService implements IRoomJoinService {
    ...
    public synchronized void confirmJoinRoom(int roomNumber, String userId) {
         ...
         */
        if(chatSessionRegistry.containsUser(String.valueOf(roomNumber), userId) || chatSessionRegistry.getRoomUserVOMap(Integer.parseInt(String.valueOf(roomNumber))) != null){
            return;
        }
    ...
    public synchronized void joinRoom(int roomNumber) {
        ...
        if (chatRoom.isEmpty()) {
            ...
            // 메모리 내 방 정보 생성(실질적 실체 생성)
            chatSessionRegistry.createRoom(roomNumber, roomConverter.toChatRoom(roomVO));

            // 대기열 방 상태를 "생성 완료"로 전환(플래그)
            roomVO.setJoined(true);
        }
        // [2] 신규/기존 방 상관 없이 세마포어 기반 permit으로 인원수 동기화
        chatSessionRegistry.updateRoomCurrentPeople(roomNumber, semaphoreRegistry.getAvailablePermits(roomNumber));
    }
}
