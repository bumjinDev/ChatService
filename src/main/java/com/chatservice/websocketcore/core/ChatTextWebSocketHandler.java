...
public class ChatTextWebSocketHandler extends TextWebSocketHandler {
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        ...
        // [1] DB/메모리 내 인원수/방 생성 갱신 (실제 인원 동기화 책임은 ChatSessionRegistry가 갖는다)
        roomJoinService.joinRoom(roomId);

        // [2] 세션 등록/상태 통합(중복 세션 강제 종료, 임시 퇴장 복귀 등) - 단일 원자적 호출
        chatSessionRegistry.handleUserSessionOnConnect(roomNumber, userId, session);

        logger.info("[입장 처리] userId={}, roomId={}", userId, roomId);

        // [3] permit 추적 정보 해제(실제 연결 시점에서만)
        ...
        countMsg.put("count",  semaphoreRegistry.getAvailablePermits(roomId) - 1);  // 인원 수 동기화
    ...
    public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
        ...
        chatSessionRegistry.handleUserSessionOnClose(roomNumber, userId, status);
        ...
        JSONObject countMsg = new JSONObject();
        countMsg.put("type", "USER_COUNT");
        countMsg.put("count", semaphoreRegistry.getAvailablePermits(roomId) -1 );   // 방 인원수 브로드 캐스팅.
        broadcast(roomNumber, new TextMessage(countMsg.toString()));
    }
