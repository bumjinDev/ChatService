#!/usr/bin/env python3
"""
Semaphore Bin별 통계 분석기
- 세마포어의 3가지 핵심 분석 영역을 bin별로 통계 집계
- 분석 1: 순차적 일관성 bin별 관찰 (규칙 1+4 통합) - 현상 관찰
- 분석 2: 동시 실행 패턴 bin별 관찰 (규칙 2 재정의) - 현상 관찰  
- 분석 3: 정원 초과 방지 bin별 검증 (규칙 3 유지) - 성공/실패 판정
"""
...
def analyze_sequential_consistency_per_bin(preprocessor_df, analysis_df, total_requests_df):
    """분석 1: 순차적 일관성 bin별 관찰 (규칙 1+4 통합) - 현상 관찰"""
    print("🔍 분석 1: 순차적 일관성 bin별 관찰 (규칙 1+4 통합) 중...")
    ...
    # bin별로 순차적 일관성 분석
    for (room_num, bin_num) in preprocessor_df.groupby(['roomNumber', 'bin']).groups.keys():
        bin_data = preprocessor_df[(preprocessor_df['roomNumber'] == room_num) & 
                                  (preprocessor_df['bin'] == bin_num)].copy()
...
def analyze_concurrent_execution_per_bin(preprocessor_df, analysis_df, total_requests_df):
    """분석 2: 동시 실행 패턴 bin별 관찰 (규칙 2 재정의) - 기존 방식으로 통일"""
    print("🔍 분석 2: 동시 실행 패턴 bin별 관찰 (규칙 2 재정의) 중...")
    ...
    print(f"  - 실제 경합 발생 레코드: {len(filtered_df)}건 (contention_group_size > 1)")
    ...
    print(f"✅ 동시 실행 패턴 bin별 분석 완료: {len(stats_df)}개 (방×bin) 조합")
    return stats_df

def analyze_capacity_prevention_per_bin(preprocessor_df, analysis_df, total_requests_df):
    """분석 3: 정원 초과 방지 bin별 검증 (규칙 3 유지) - 성공/실패 판정"""
    print("🔍 분석 3: 정원 초과 방지 bin별 검증 (규칙 3 유지) 중...")
    ...
    if len(capacity_exceeded) > 0:
        # (roomNumber, bin) 단위로 그룹화하여 통계 계산
        grouped = capacity_exceeded.groupby(['roomNumber', 'bin'])
        
        for (room_num, bin_num), group in grouped:
            # 해당 bin의 모든 초과 인원 값들 추출
    ...
    print(f"✅ 정원 초과 방지 bin별 검증 완료: {len(result_stats)}개 (방×bin) 조합")
    return result_stats
...
def create_excel_output(sequential_df, concurrent_df, capacity_df, output_file):
    ...
    # 시트 1: 순차적 일관성 bin별 분석
    ws1 = wb.create_sheet("Bin_Sequential_Analysis")
    add_dataframe_to_sheet(ws1, sequential_df, "Bin Analysis 1: Sequential Consistency")
    
    # 시트 2: 동시 실행 패턴 bin별 분석 (시트명 변경: 기존과 통일)
    ws2 = wb.create_sheet("Bin_Contention_Analysis")
    add_dataframe_to_sheet(ws2, concurrent_df, "Bin Analysis 2: Contention")
    
    # 시트 3: 정원 초과 방지 bin별 검증
    ws3 = wb.create_sheet("Bin_Capacity_Analysis")
    add_dataframe_to_sheet(ws3, capacity_df, "Bin Analysis 3: Capacity Prevention")
    
    wb.save(output_file)
    print(f"✅ 세마포어 bin별 Excel 파일 저장 완료: {output_file}")
    print("  📋 생성된 시트:")
    print("    - Bin_Sequential_Analysis: 순차적 일관성 bin별 관찰")
    print("    - Bin_Contention_Analysis: 동시 실행 패턴 bin별 관찰 (기존과 통일)")
    print("    - Bin_Capacity_Analysis: 정원 초과 방지 bin별 검증")

def print_summary_statistics(sequential_df, concurrent_df, capacity_df, total_requests_df):
    """분석 결과 요약 통계 출력"""
    print("\n" + "="*80)
    print("📈 SEMAPHORE BIN별 통계 분석 결과 요약")
    print("="*80)
    ...
    if len(sequential_df) > 0:
        ...
        affected_bins_seq = len(sequential_df[sequential_df['순차적 일관성 차이 건수'] > 0])
        print(f"분석 1 (순차적 일관성 관찰): {affected_bins_seq}개 bin에서 {sequential_differences}건 차이 관찰")
    ...
    if len(concurrent_df) > 0:
        ...
        affected_bins_con = len(concurrent_df[concurrent_df['발생 건수'] > 0])
        print(f"분석 2 (동시 실행 패턴 관찰): {affected_bins_con}개 bin에서 {concurrent_executions}건 경합 관찰 (최대 {max_concurrent_level}개 스레드)")
    
    # 분석 3: 정원 초과
    if len(capacity_df) > 0:
        capacity_exceeded = capacity_df['정원 초과 발생 건수'].sum()
        affected_bins_cap = len(capacity_df[capacity_df['정원 초과 발생 건수'] > 0])
        print(f"분석 3 (정원 초과 방지 검증): {affected_bins_cap}개 bin에서 {capacity_exceeded}건 초과 발생")
    
    print(f"\n--- 세마포어 종합 평가 ---")
    if len(capacity_df) > 0 and capacity_df['정원 초과 발생 건수'].sum() == 0:
        print("🎉 세마포어 핵심 기능 (정원 초과 방지): 모든 bin에서 완벽 성공")
    elif len(capacity_df) > 0:
        print("⚠️ 세마포어 핵심 기능 (정원 초과 방지): 일부 bin에서 검증 실패")
...
def main():
    """메인 함수"""
    parser = argparse.ArgumentParser(description="세마포어 bin별 통계 분석기")
    ...
    parser.add_argument('output_xlsx', help='세마포어 bin별 통계 분석 Excel 출력 파일')
    parser.add_argument('--rooms', help='분석할 방 번호 (쉼표로 구분)')
    
    args = parser.parse_args()
    
    try:
        print("🚀 세마포어 bin별 통계 분석기 시작...")
        print("🎯 세마포어 3가지 bin별 분석: 순차적 일관성 관찰 + 동시 실행 패턴 관찰 + 정원 초과 방지 검증")
        ...
        # 4. 세마포어 3가지 bin별 분석
        print("\n=== 🎯 세마포어 3가지 bin별 분석 실행 ===")
        sequential_df = analyze_sequential_consistency_per_bin(preprocessor_df, analysis_df, total_requests_df)
        concurrent_df = analyze_concurrent_execution_per_bin(preprocessor_df, analysis_df, total_requests_df)
        capacity_df = analyze_capacity_prevention_per_bin(preprocessor_df, analysis_df, total_requests_df)
