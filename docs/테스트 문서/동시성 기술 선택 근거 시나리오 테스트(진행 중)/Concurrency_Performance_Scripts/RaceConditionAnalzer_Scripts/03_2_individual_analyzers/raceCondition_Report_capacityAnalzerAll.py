...
class Rule3CapacityAnalyzer:
    def __init__(self, room_number=None, preprocessor_file=None, result_file=None, output_dir=None):
        ...
        Args:
            ...
            result_file (str): 분석 결과 데이터 파일 경로 (detected_anomalies.csv - CSV용)
    ...
    def _create_rule3_single_room_chart(self):
        ...
        max_exceeded = 0
        if exceeded_count > 0 and 'anomaly_type' in self.df_result.columns:
            capacity_errors = self.df_result[
                self.df_result['anomaly_type'].str.contains('정원 초과 오류', na=False)
            ]
    ...
    def generate_rule3_csv_report(self):
        """규칙 3 전체 스레드 CSV 보고서 생성 (오류 여부 무관, 모든 스레드 포함)"""
        print("📋 Rule3 전체 스레드 CSV 보고서 생성 시작")
        ...
        # 방 번호 필터링 (이중 확인)
        if self.room_number is not None:
            before_room_filter = len(all_thread_data)
            all_thread_data = all_thread_data[all_thread_data['roomNumber'] == self.room_number]
            print(f"   - 방 {self.room_number} 필터링: {before_room_filter} → {len(all_thread_data)}건")
        
        # 파일명 생성 (모든 스레드 포함을 명시)
        if self.room_number:
            csv_filename = f'report_rule3_all_threads_room{self.room_number}_complete.csv'
        else:
            csv_filename = 'report_rule3_all_threads_complete.csv'
    ...
    def run_analysis(self):
        ...
        print("✅ Rule 3 분석 완료! (전체 스레드 데이터 - 원본 순서 유지)")
        return True
