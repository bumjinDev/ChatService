...
class Rule1LostUpdateAnalyzer:
    ...
    def generate_rule1_csv_report(self):
        """규칙 1 전체 스레드 CSV 보고서 생성 (오류 여부 무관, 모든 스레드 포함)"""
        print("📋 Rule1 전체 스레드 CSV 보고서 생성 시작")
        ...
        if self.room_number:
            csv_filename = f'report_rule1_all_threads_room{self.room_number}_complete.csv'
        else:
            csv_filename = 'report_rule1_all_threads_complete.csv'
        ...
        if all_thread_data.empty:
            ...
            print(f"   - 오류 여부와 상관없이 모든 스레드 데이터 포함")
